//! File logging for the Windows daemon.
//!
//! The daemon is spawned with CREATE_NO_WINDOW, so stderr goes nowhere: without this,
//! every daemon-side failure — credential writes included — is invisible anywhere.
//! The log lives at `%LOCALAPPDATA%\tidemark\logs\daemon.log`; when the directory
//! cannot be created the daemon falls back to stderr-only rather than refusing to start.
...
/// Opens `%LOCALAPPDATA%\tidemark\logs\daemon.log` for appending. `None` means
/// logging stays stderr-only.
