...
use tidemark_core::providers::keyed::{self, openai_api, openrouter, poe, xai};
...
/// request, so a `keyed::Spec` cannot describe them — OpenAI pages two Admin API
/// endpoints, OpenRouter reads credits and key quota, Poe pages through a usage
/// history, xAI reads a prepaid balance and a spend history. Each entry is the
/// provider's own [`keyed::HandSpec`], which carries everything a `Spec` carries
/// except the single endpoint, and says how to build a client from the stored key and
/// the account's settings. The credential is the same pasted key as the catalog's,
/// `CredentialKind::Key`, so the credentials dialog is unchanged.
static HAND_WRITTEN: &[&keyed::HandSpec] =
    &[&openai_api::SPEC, &openrouter::SPEC, &poe::SPEC, &xai::SPEC];
