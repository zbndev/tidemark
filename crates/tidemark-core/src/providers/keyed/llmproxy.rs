...
//! is a *required* free-text option and [`Keyed::new`](super::Keyed::new) refuses to
//! build without one, naming the setting. The shared reader enforces HTTPS — plain
//! HTTP stands for loopback only, which is how a local instance is reached.
//! `/v1/quota-stats` is appended as the fetcher appends it: a base already ending in
//! `/v1` is left alone.
//!
//! # The meaning
//!
...
/// LLM Proxy as the keyed mechanism sees it.
